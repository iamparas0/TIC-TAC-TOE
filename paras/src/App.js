...
const App = () => {
  ...
  }, [board, currentPlayer, winner, draw]);

  useEffect(() => {
    if (isComputerMode && currentPlayer === 'O' && !winner && !draw) {
      const timer = setTimeout(() => {
        makeComputerMove();
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [currentPlayer, isComputerMode, winner, draw]);
  ...
  const toggleComputerMode = () => {
    setIsComputerMode((prevMode) => !prevMode);
    resetGame();
  };
    ...
    <div className={`app ${isDarkMode ? 'dark' : ''}`}>
      ...
      <div className="rules-card">
        ...
        <ul>
          ...
          <li>Toggle the computer mode to play against the computer (you'll be X).</li>
        </ul>
      </div>
